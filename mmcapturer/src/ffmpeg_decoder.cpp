...
bool FFmpegDecoder::init(enum AVCodecID id)
{
	...
	LOG_DEBUG("init hardware decoder: %d", m_hw_available);
...
bool FFmpegDecoder::send_video_data(uint8_t* data, size_t size, long long timestamp)
{
	...
	if (!m_frame)
	{
		...
		if (!m_frame)
		{
			LOG_ERROR("av_frame_alloc() return NULL");
			return false;
		}
	}
...
bool FFmpegDecoder::scale_frame()
{
	...
	if (m_sws_context)
	{
		if (!m_sws_frame)
		{
			...
			if (ret < 0)
			{
				LOG_ERROR("av_image_get_buffer_size() fail");
				return false;
			}
			m_sws_frame_buffer = (uint8_t *)av_malloc(ret);
			if (!m_sws_frame_buffer)
			{
				LOG_ERROR("av_malloc return NULL");
				return false;
			}
			...
			if (ret < 0)
			{
				LOG_ERROR("av_image_fill_arrays fail");
				return false;
			}
		}
